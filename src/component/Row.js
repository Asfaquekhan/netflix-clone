...
export default function Row({ title, fetchURL, rowID}) {
  ...
  return (
    <>
      ...
      <MdChevronLeft size={40} className='bg-white rounded-full absolute opacity-50 hover:opacity-100 cursor-pointer z-10 hidden text-black group-hover:block' onClick={slideleft}/>
        ...
        <MdChevronRight size={40} className='bg-white rounded-full absolute right-0 opacity-50 hover:opacity-100 cursor-pointer z-10 hidden text-black group-hover:block'onClick={slideright}/>
      </div>
    </>
  );
}
